...
console.log("Total Profits/Losses over entire period: $" + totalProfit); 


// 3. The net total amount of Profit/Losses over the entire period.

var profitChange = finances [1][1] - finances [0][1];

for (var i = 2; i < finances.length; i++) {
    profitChange += finances[i][1] - finances[i-1][1];
} //add difference between monthly profits to old profit

var avgProfitChange = profitChange / (finances.length - 1);
console.log("Average change: £" + avgProfitChange.toFixed(2));


// 4. The greatest increase in profits (date and amount) over the entire period.

var maxProfitChange = finances[1][1] - finances[0][1];
var maxProfitMonth = finances[1][0];  // the first month profit is calculated from
...
console.log ("Greatest Increase in Profits: " + maxProfitMonth + " (£" + maxProfitChange + ") ");
...
for (var i = 2; i < finances.length; i++) {
    ...
    } //check first profit change and replaces if next change is lower than initial value
}

console.log("Greatest Decrease in Profits: " + minProfitMonth + " (£" + minProfitChange + ") ");